...
@login_required
def alunos_list():
    # DIRETORIA / PROFESSOR: veem todos
    if current_user.is_diretoria() or current_user.is_professor():
        items = Aluno.query.order_by(Aluno.nome.asc()).all()
    else:
        # RESPONSAVEL / ALUNO: só o aluno vinculado
        if current_user.aluno_id:
            aluno = db.session.get(Aluno, current_user.aluno_id)
            items = [aluno] if aluno else []
        else:
            items = []
