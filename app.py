...
from sqlalchemy import UniqueConstraint, ForeignKey, text, inspect
...
# Expor view_functions para Jinja
...
class UserSession(db.Model):
    ...
    def marcar_seen(self): self.ultimo_seen = datetime.utcnow()
...
class AlunoExtra(db.Model):
    ...
    dados = db.Column(db.Text, nullable=False, default="{}")
...
class Horario(db.Model):
    ...
    label = db.Column(db.String(20), nullable=False, default="")  # pode faltar em DB antigo
    criado_em = db.Column(db.DateTime, default=datetime.utcnow)
...
class Atividade(db.Model):
    ...
    data_atividade = db.Column(db.String(10))
...
def usuario_tem_permissao(usuario, codigo):
...
def registrar_login_sessao(usuario):
    sid = flask_session.get("session_id")
    if not sid:
        sid = str(uuid.uuid4())
...
def autoupgrade_schema():
    """
    Corrige DBs antigos que não têm a coluna horario.label.
    """
    with db.engine.begin() as conn:
        insp = inspect(conn)
        if "horario" in insp.get_table_names():
            cols = [c["name"] for c in insp.get_columns("horario")]
            if "label" not in cols:
                backend = conn.engine.url.get_backend_name()
                if backend.startswith("sqlite"):
                    conn.execute(text("ALTER TABLE horario ADD COLUMN label VARCHAR(20)"))
                else:
                    conn.execute(text("ALTER TABLE horario ADD COLUMN label VARCHAR(20) NOT NULL DEFAULT ''"))
                # Preencher label
                if backend.startswith("sqlite"):
                    conn.execute(text("UPDATE horario SET label = (hora_inicio || ' - ' || hora_fim) WHERE (label IS NULL OR label='')"))
                else:
                    conn.execute(text("UPDATE horario SET label = CONCAT(hora_inicio, ' - ', hora_fim) WHERE (label IS NULL OR label='')"))
...
def usuarios_list():
    ...
    for u in usuarios:
        if u.papel == "DIRETORIA":
            permissoes_por_usuario[u.id] = {c: True for c in PERMISSOES_CODIGOS}
        else:
            marcadas = set(
                p.codigo for p in (
                    Permissao.query
            ...
            permissoes_por_usuario[u.id] = {c: (c in marcadas) for c in PERMISSOES_CODIGOS}
...
def usuarios_salvar_permissoes(usuario_id):
    if current_user.papel != "DIRETORIA": abort(403)
    ...
    for cod in enviados:
        if cod not in PERMISSOES_CODIGOS: abort(400)
    atuais = {
        ...
        .all() if up is not None and up.usuario_id == u.id
    }
    cache_perm = {p.codigo: p for p in Permissao.query.all()}
    for cod, up in list(atuais.items()):
        if cod not in enviados: db.session.delete(up)
    for cod in enviados:
        if cod not in atuais:
            perm = cache_perm.get(cod)
            if perm: db.session.add(UsuarioPermissao(usuario_id=u.id, permissao_id=perm.id))
    db.session.commit()
...
def usuarios_novo():
    ...
    if request.method == "POST":
        email = request.form.get("email","").strip().lower()
        senha = request.form.get("senha",""); confirmar = request.form.get("confirmar","")
        papel = request.form.get("papel","RESPONSAVEL")
        if not email or not senha or not confirmar:
            flash("Preencha todos os campos.", "warning"); return render_template("usuario_form.html")
        if senha != confirmar:
            flash("A confirmação de senha não confere.", "warning"); return render_template("usuario_form.html")
        if len(senha) < 8 or not any(ch.isdigit() for ch in senha):
            flash("Senha deve ter ao menos 8 caracteres e 1 dígito.", "warning"); return render_template("usuario_form.html")
        if Usuario.query.filter_by(email=email).first():
            flash("E-mail já cadastrado.", "warning"); return render_template("usuario_form.html")
        u = Usuario(email=email, papel=papel, ativo=True); u.set_password(senha)
...
def _parse_hhmm(val: str) -> dtime | None:
    try:
        hh, mm = val.split(":"); return dtime(hour=int(hh), minute=int(mm))
    except Exception:
        return None
def _get_bool(val): return True if str(val).lower() in ("1","true","on","sim") else False
...
def alunos_novo():
    ...
    if request.method == "POST":
        ...
        horario = request.form.get("horario_sel","") or request.form.get("horario_txt","").strip()
        telefone_mae = request.form.get("telefone_mae","").strip()

        foto_file = request.files.get("foto"); foto_path = None
        if foto_file and foto_file.filename:
            fname = f"{uuid.uuid4().hex}_{foto_file.filename}"
            foto_file.save(os.path.join(UPLOAD_DIR, fname)); foto_path = fname
        ...
        db.session.add(aluno); db.session.flush()
...
def alunos_editar(id):
    ...
    if request.method == "POST":
        ...
        if foto_file and foto_file.filename:
            fname = f"{uuid.uuid4().hex}_{foto_file.filename}"
            foto_file.save(os.path.join(UPLOAD_DIR, fname)); aluno.foto_path = fname
...
def alunos_excluir(id):
    aluno = Aluno.query.get_or_404(id)
    db.session.delete(aluno); db.session.commit()
...
def alunos_search():
    ...
    qry = Aluno.query
    if q: qry = qry.filter(Aluno.nome.ilike(f"%{q}%"))
...
def escolas_novo():
    if request.method == "POST":
        nome = request.form.get("nome","").strip()
        if not nome:
            flash("Informe o nome da escola.", "warning"); return render_template("escolas/novo.html")
        if Escola.query.filter(Escola.nome.ilike(nome)).first():
            flash("Já existe uma escola com esse nome.", "warning"); return render_template("escolas/novo.html")
        db.session.add(Escola(nome=nome)); db.session.commit()
...
def escolas_editar(id):
    ...
    if request.method == "POST":
        nome = request.form.get("nome","").strip()
        if not nome:
            flash("Informe o nome da escola.", "warning"); return render_template("escolas/editar.html", escola=e)
        existe = Escola.query.filter(Escola.nome.ilike(nome), Escola.id != e.id).first()
        if existe:
            flash("Já existe outra escola com esse nome.", "warning"); return render_template("escolas/editar.html", escola=e)
        e.nome = nome; db.session.commit()
        flash("Escola atualizada!", "success"); return redirect(url_for("escolas_listar"))
...
def series_novo():
    if request.method == "POST":
        nome = request.form.get("nome","").strip()
        if not nome:
            flash("Informe o nome da série.", "warning"); return render_template("series/novo.html")
        if Serie.query.filter(Serie.nome.ilike(nome)).first():
            flash("Já existe uma série com esse nome.", "warning"); return render_template("series/novo.html")
        db.session.add(Serie(nome=nome)); db.session.commit()
...
def series_editar(id):
    ...
    if request.method == "POST":
        nome = request.form.get("nome","").strip()
        if not nome:
            flash("Informe o nome da série.", "warning"); return render_template("series/editar.html", serie=s)
        existe = Serie.query.filter(Serie.nome.ilike(nome), Serie.id != s.id).first()
        if existe:
            flash("Já existe outra série com esse nome.", "warning"); return render_template("series/editar.html", serie=s)
        s.nome = nome; db.session.commit()
        flash("Série atualizada!", "success"); return redirect(url_for("series_listar"))
...
def horarios_listar():
    horarios = Horario.query.order_by(Horario.hora_inicio.asc()).all()
    return render_template("horarios_listar.html", horarios=horarios)
...
def horarios_novo():
    if request.method == "POST":
        ...
        t_ini = _parse_hhmm(h_ini); t_fim = _parse_hhmm(h_fim)
        if not t_ini or not t_fim:
            flash("Informe horas válidas no formato HH:MM.", "warning"); return render_template("horarios/novo.html")
        if t_fim <= t_ini:
            flash("A hora fim deve ser MAIOR que a hora início.", "danger"); return render_template("horarios/novo.html")
        label = f"{h_ini} - {h_fim}"
        db.session.add(Horario(hora_inicio=h_ini, hora_fim=h_fim, label=label))
        db.session.commit()
        flash("Horário cadastrado!", "success"); return redirect(url_for("horarios_listar"))
...
def horarios_editar(id):
    ...
    if request.method == "POST":
        ...
        t_ini = _parse_hhmm(h_ini); t_fim = _parse_hhmm(h_fim)
        if not t_ini or not t_fim:
            flash("Informe horas válidas no formato HH:MM.", "warning"); return render_template("horarios/editar.html", horario=h)
        if t_fim <= t_ini:
            flash("A hora fim deve ser MAIOR que a hora início.", "danger"); return render_template("horarios/editar.html", horario=h)
        h.hora_inicio = h_ini; h.hora_fim = h_fim; h.label = f"{h_ini} - {h_fim}"
        db.session.commit()
        flash("Horário atualizado!", "success"); return redirect(url_for("horarios_listar"))
...
def atividades_listar():
    atividades = Atividade.query.order_by(Atividade.criado_em.desc()).all()
...
def atividades_novo():
    if request.method == "POST":
        ...
        db.session.add(Atividade(aluno_id=int(aluno_id),
...
def atividades_excluir(id):
    atv = Atividade.query.get_or_404(id)
    db.session.delete(atv); db.session.commit()
...
def init_db_if_needed():
    with app.app_context():
        db.create_all()
        autoupgrade_schema()  # <<<<<<<<<<<<<<<<<<<< AQUI FAZ O UPGRADE DA COLUNA LABEL
        seed_permissoes()
        seed_admin()
